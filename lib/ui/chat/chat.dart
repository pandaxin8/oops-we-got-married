...
class _ChatPageState extends State<ChatPage> {
  ...
  void _onGenerateScenarioPressed() async {
    // This method will be called when the user presses the "Generate Scenario" button.
    try {
      final scenarioText = await _geminiService.generateScenario(
        // Pass the actual player names, interests, and moods here.
        ['Alice', 'Bob'],
        ['tea', 'chess'],
        ['happy', 'curious'],
      );
      _addScenarioToChat(scenarioText);
    } catch (e) {
  ...
  void _addScenarioToChat(String scenarioText) {
    final scenarioMessage = types.TextMessage(
      author: types.User(id: 'game-master'), // You can define a 'game master' user in your system.
      createdAt: DateTime.now().millisecondsSinceEpoch,
      id: const Uuid().v4(),
      text: scenarioText,
    );
    _addMessage(scenarioMessage);
  }
